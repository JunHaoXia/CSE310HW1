...
def createRequest(host, url, originalRequest):
    try:
        hostIndex = url.index(host)
        if len(url[hostIndex + len(host):]) < 3:
            url = 'http:/' + url
            pass
        else:
            url = url[hostIndex + len(host):]
    except ValueError:
        pass
